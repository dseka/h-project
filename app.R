...
library(htmlwidgets)
url <- "https://github.com/dseka/h-project/raw/master/CIV_adm2.sf.rds"
temp<- tempfile()
download.file(url, temp)
new_civ <- readRDS(temp)
unlink(temp)
new_civ$NAME_2<-ifelse(Encoding(new_civ$NAME_2)=="UTF-8", iconv(new_civ$NAME_2,from="UTF-8",to="ASCII//TRANSLIT"), new_civ$NAME_2)
...
ui <- fluidPage(style="padding:0px",
                ...
                fluidRow(
                    column(width=3, 
                           wellPanel(style="background-color:#212121; padding:10px; text-align:center; color:#777777; height:270px; border-radius: 10px; border: 2px solid #2A2A2A; font-size:1vw;",  
                                     ...
                                     span(style="color:red; font-size:1.6vw;", textOutput("wt")),
                                     hr(style="width:50%; border-color:teal"),
                                     
                                     span(style="font-size:1vw;color: #777777",    "Active Cases"), 
                                     
                                     span(style="color:red; font-size:1.6vw;", textOutput("wa")),
                                   
                           ),
                           wellPanel(style="background-color:#212121; padding:10px; text-align:center; color:#B1B1B1; border-radius: 10px; height:370px; border: 2px solid #2A2A2A; font-size:1.6vw;",  
...
server<-function(input, output){
    ...
    output$totcases<-renderGirafe({
        map_ci=ggplot() +
            ...
            geom_point_interactive(data=nciv, mapping=aes(x=long, y=lat, group=NAME_2, size=ci_covid[,ncol(ci_covid)], tooltip=tooltip_point), fill="red", alpha=0.3, shape=21) + 
